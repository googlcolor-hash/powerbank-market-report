...
base=repo/'assets'/'telegram_refs_misha_recent'
base.mkdir(parents=True, exist_ok=True)
media_root=base/'media'; media_root.mkdir(exist_ok=True)
jsonl=base/'messages_misha_recent.jsonl'
...
async def main():
    ...
    day_start_utc=datetime(2026,5,20,0,0,0,tzinfo=msk).astimezone(timezone.utc)

    rows=[]
    async for msg in client.iter_messages(entity, limit=2500):
        if not msg.date: continue
        dt=msg.date.astimezone(timezone.utc)
        if dt < day_start_utc: break
        sender=''
        if msg.sender is not None:
            sender=get_display_name(msg.sender) or ''
        if 'Миша Ломан' not in sender: continue
        k=kind_of(msg)
        rel=''
        if k!='none':
            sub=media_root/k; sub.mkdir(parents=True, exist_ok=True)
            p=await client.download_media(msg, file=str(sub/f'msg_{msg.id}'))
            if p: rel=str(Path(p).resolve().relative_to(base.resolve())).replace('\\','/')
    ...
    for r in rows:
        ...
        md=''
        if r['media']:
            rel='assets/telegram_refs_misha_recent/'+r['media']
    ...
    content=''.join(cards) if cards else '<p>Новых сообщений от Миши не найдено.</p>'
    page=f"<!doctype html><html lang='ru'><head><meta charset='utf-8'><meta name='viewport' content='width=device-width,initial-scale=1'><title>Миша: последние референсы</title><style>body{{margin:0;font-family:Inter,Segoe UI,Arial,sans-serif;background:#0c111b;color:#e9eef9}}.wrap{{max-width:1200px;margin:0 auto;padding:24px}}.hero{{padding:20px;border:1px solid #2a3550;border-radius:14px;background:linear-gradient(135deg,#111b2d,#0f1420)}}h1{{margin:0 0 8px;font-size:34px}}.muted{{color:#a4b3cf}}.grid{{margin-top:18px;display:grid;grid-template-columns:repeat(auto-fill,minmax(320px,1fr));gap:14px}}.card{{background:#111827;border:1px solid #263247;border-radius:12px;padding:12px}}.head{{display:flex;justify-content:space-between;gap:8px;margin-bottom:6px}}.meta{{font-size:12px;color:#96a6c3;margin-bottom:8px}}.text{{font-size:14px;white-space:pre-wrap;line-height:1.45;margin-bottom:10px}}img,video{{width:100%;border-radius:8px;border:1px solid #2a3952;background:#000}}audio{{width:100%}}a{{color:#86d7ff}}</style></head><body><div class='wrap'><section class='hero'><h1>Референсы от Миши</h1><div class='muted'>Чат 6340811092 • период: с 20.05.2026 • сообщений: {len(rows)}</div><p><a href='index.html'>На главную</a></p></section><section class='grid'>{content}</section></div></body></html>"
    (repo/'telegram-refs-misha-recent.html').write_text(page,encoding='utf-8')
